...
public interface IInventoryRepository extends JpaRepository<Inventory, Long> {
    ...
    @Query("SELECT i  FROM Inventory i WHERE i.productCard.id = :cardId")
    List<Inventory> getByCardId(@Param("cardId") long cardId);

    @Transactional
    @Modifying
    @Query("DELETE FROM Inventory i WHERE i.productCard.id = :id")
    void deleteByCardId(@Param("id") long id);

    @Query("SELECT i FROM Inventory i WHERE i.productCard.id = :cardId AND i.color = :color AND i.size = :size")
